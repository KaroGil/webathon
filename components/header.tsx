...
import { getInterestBySlug } from "@/components/types/interests";
import { getEventById } from "@/components/types/post";
import type { Event } from "@/components/types/post";
  ...
  useEffect(() => {
    const updateTitle = async () => {
      if (pathname.startsWith("/hobbies/")) {
        ...
        const interest = await getInterestBySlug(hobby);
        if (interest) {
          setTitle(`${interest.infinitiv.toUpperCase()}kompis`);
        }
      } else if (pathname.startsWith("/event/")) {
        const id = parseInt(pathname.split("/")[2]);
        const event: Event | undefined = await getEventById(id);
        if (event) {
